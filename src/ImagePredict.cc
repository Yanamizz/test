...
static float ApplyErrorVelocityFeedforward(float measured_velocity_deg_per_sec,
  ...
  const float safe_limit =
      static_cast<float>(std::max(0.0, velocity_abs_limit));
...
void ImagePredictThread(ImageRecognize::ImagePredict &predictor,
  ...
  while (g_running) {
    ...
    if (g_aimbot_laser_state_controller.CurrentStage() >=
    ...
    if (has_tracked_box) {
      ...
      const auto distance_debug =
          distance_calculator.CalculateDistanceWithDebug(
      ...
      if (std::isfinite(distance) && distance > 0.0f) {
        ...
        overlay_data.show_shadow_distance_debug = true;
        overlay_data.shadow_width_distance =
            distance_debug.legacy_width_distance;
        overlay_data.shadow_height_distance =
            distance_debug.legacy_height_distance;
        overlay_data.shadow_width_valid =
            std::isfinite(distance_debug.legacy_width_distance) &&
            distance_debug.legacy_width_distance > 0.0f;
        overlay_data.shadow_height_valid =
            std::isfinite(distance_debug.legacy_height_distance) &&
            distance_debug.legacy_height_distance > 0.0f;
      }
      ...
      if (has_matched_imu) {
        ...
        const float laser_pitch_comp_deg = laser_pitch_comp_stabilizer.Filter(
            laser_pitch_comp_deg_raw, has_realtime_frame_dt ? frame_dt : 0.0);
    ...
    const bool full_run_overlay_due = enable_save_full_run_video &&
