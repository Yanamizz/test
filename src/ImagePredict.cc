...
static std::atomic<int> g_aerial_robot_stage{ImageRecognize::AerialRobotLaserLockJudge::kInitialStage};
...
static void DecrementAimbotTargetSaturated() {
  ...
  while (old_value > SerialTask::kAimbotTargetMin) {
    if (g_aimbot_target.compare_exchange_weak(old_value, static_cast<uint8_t>(old_value - 1), std::memory_order_acq_rel,
...
static double EffectiveScanSendHz() { return std::clamp(Tools::Params().scan_send_hz, 1.0, MaxSerialAimbotFrameHz()); }
...
static void WaitForScanStateChangeFor(std::chrono::milliseconds timeout) {
  ...
  g_pending_send_cv.wait_for(lk, timeout, []() {
    return !g_running.load(std::memory_order_acquire) || !g_send_is_scan.load(std::memory_order_acquire) ||
           g_has_pending_send || g_target_visible.load(std::memory_order_acquire);
  });
}
...
static void SendAimbotCommand(serial::Serial &port, const AimbotSendCommand &command) {
  SerialTask::SerialSend(port, command.absolute_pitch, command.absolute_yaw, command.offset_pitch, command.offset_yaw,
                         ...
                         g_aimbot_target.load(std::memory_order_acquire));
}
...
static void PrintPredictSettings(Tools::FilterType filter_type, ImageRecognize::TargetCampMode target_camp_mode,
  ...
  if (effective_scan_send_hz < Tools::Params().scan_send_hz) {
    std::cout << "（配置值 " << Tools::Params().scan_send_hz << "，已受串口带宽限制）";
  }
...
static void AddLatencySample(bool enabled, LatencyStats &total, LatencyStats &window,
                             Tools::LatencyBucket LatencyStats::*bucket, const std::chrono::steady_clock::time_point &t0,
...
static void UpdateAerialRobotStageAndSwitchFlag(bool has_predict_result, const ImageRecognize::PredictResult &result,
  ...
  if (!has_predict_result || g_aerial_robot_stage.load(std::memory_order_acquire) >=
  ...
  if (stage >= ImageRecognize::AerialRobotLaserLockJudge::kFinishedStage &&
      previous_stage < ImageRecognize::AerialRobotLaserLockJudge::kFinishedStage) {
    *pending_stage3_switch = true;
    std::cout << "[空中机器人阶段] 达到 stage=3，等待目标丢失持续 "
              << stage3_switch_target_lost_delay.count() << "ms 后切换模型" << std::endl;
  }
}

static void UpdateTargetVisibleAndLostSince(bool track_alive, const std::chrono::steady_clock::time_point &now,
...
static bool Stage3SwitchTargetLostLongEnough(bool target_lost_since_initialized,
                                             const std::chrono::steady_clock::time_point &now,
                                             const std::chrono::steady_clock::time_point &target_lost_since,
                                             const std::chrono::milliseconds &stage3_switch_target_lost_delay) {
  return target_lost_since_initialized && (now - target_lost_since) >= stage3_switch_target_lost_delay;
}
...
int main(int argc, char **argv) {
  ...
  g_exposure_controller.SetExposureTimes(Tools::Params().stage12_exposure_time_us,
  ...
  try {
    predictor = std::make_unique<ImageRecognize::ImagePredict>(Tools::Params().stage12_model_path,
                                                               Tools::Params().openvino_device_name);
  } catch (const std::exception &e) {
    std::cerr << "初始化 OpenVINO 模型失败：" << e.what() << std::endl;
    std::cerr << "当前 stage12_model_path: " << Tools::Params().stage12_model_path << std::endl;
...
void CaptureThread(CameraTask::GalaxyCamera *camera, std::optional<std::string> video_path) {
  ...
  while (g_running) {
    ...
    if (frame.empty()) {
      std::this_thread::sleep_for(std::chrono::milliseconds(Tools::Params().capture_empty_sleep_ms));
      continue;
    }
...
void ImagePredictThread(ImageRecognize::ImagePredict &predictor, Tools::ScanController &scan_controller,
  ...
  PrintPredictSettings(filter_type, target_camp_mode, enable_display, enable_scan_mode, enable_save_no_target_images,
                       enable_save_target_videos, enable_latency_profile, enable_calibration_sliders, enable_send_log);
  ...
  const auto stage3_switch_target_lost_delay =
      std::chrono::milliseconds(std::max(0, Tools::Params().stage3_switch_target_lost_delay_ms));
  std::chrono::steady_clock::time_point target_lost_since{};
  ...
  const auto switch_to_stage3_predictor = [&](const char *reason) -> bool {
    ...
    try {
      g_exposure_controller.SetActiveMode(CameraTask::ExposureHotkeyController::ExposureMode::Stage3);
      Tools::DistanceCalculator::SetActiveStage(Tools::CalibrationStage::Stage3);
      Tools::LaserAngleCalculator::SetActiveStage(Tools::CalibrationStage::Stage3);
      if (!stage3_predictor) {
        stage3_predictor = std::make_unique<ImageRecognize::ImagePredict>(Tools::Params().stage3_model_path,
                                                                          Tools::Params().openvino_device_name);
      }
      ...
      std::cout << "[空中机器人阶段] 切换到 stage3，模型=" << Tools::Params().stage3_model_path
                << " 曝光(us)=" << Tools::Params().stage3_exposure_time_us << " 原因=" << reason << std::endl;
      return true;
    } catch (const std::exception &e) {
  ...
  const auto switch_to_stage12_predictor = [&](const char *reason) {
    ...
    g_exposure_controller.SetActiveMode(CameraTask::ExposureHotkeyController::ExposureMode::Stage12);
    Tools::DistanceCalculator::SetActiveStage(Tools::CalibrationStage::Stage12);
    Tools::LaserAngleCalculator::SetActiveStage(Tools::CalibrationStage::Stage12);
    active_predictor = &predictor;
    using_stage3_predictor = false;
    pending_stage3_switch = false;
    reset_tracking_state();
    std::cout << "[空中机器人阶段] 切换到 stage1/2，模型=" << Tools::Params().stage12_model_path
              << " 曝光(us)=" << Tools::Params().stage12_exposure_time_us << " 原因=" << reason << std::endl;
  };
  ...
  while (g_running) {
    ...
    double frame_dt = 0.0;
    double stage_dt = ImageRecognize::AerialRobotLaserLockJudge::kDefaultDeltaSeconds;
    ...
    try {
      ...
      if (!has_realtime_frame_dt || frame_dt <= 0.0 || frame_dt > Tools::Params().dt_max_sec) frame_dt = 0.0;
    ...
    UpdateAerialRobotStageAndSwitchFlag(has_predict_result, result, stage_dt, &aerial_robot_stage_judge,
                                        &pending_stage3_switch, stage3_switch_target_lost_delay);

    std::array<float, 6> tracked_box{};
    bool has_tracked_box = false;
    const auto track_boxes = ImageRecognize::FilterTrackBoxes(result.boxes, target_camp_mode);
    ...
    AddLatencySample(enable_latency_profile, latency_total, latency_window, &LatencyStats::select_box_ns, t_select_start,
                     t_select_end);
    const bool track_alive = track_result.has_box || target_tracker.HasRecentLock();
    const auto now = std::chrono::steady_clock::now();
    UpdateTargetVisibleAndLostSince(track_alive, now, &target_lost_since, &target_lost_since_initialized);

    const bool stage3_switch_target_lost_long_enough = Stage3SwitchTargetLostLongEnough(
        target_lost_since_initialized, now, target_lost_since, stage3_switch_target_lost_delay);
    if (pending_stage3_switch && !using_stage3_predictor && !track_alive && stage3_switch_target_lost_long_enough) {
    ...
    if (has_tracked_box) {
      ...
      const auto &distance_box = track_result.has_box ? track_result.box : tracked_box;
      const float height = ImageRecognize::BoxHeight(distance_box);
      pixel_height_stats.Add(height);
      const float distance =
          distance_calculator.CalculateDistance(distance_box[0], distance_box[1], distance_box[2], distance_box[3]);
      ...
      if (has_matched_imu) {
        ...
        auto [laser_yaw_angle, laser_pitch_angle] =
            laser_angle_calculator.CalculateLaserAngles(distance, offset_yaw_angle, offset_pitch_angle);

        const auto t_control_start = ProfileNow(enable_latency_profile);
        float delta_yaw_raw = Tools::NormalizeDeltaDeg(static_cast<float>(offset_yaw_angle + laser_yaw_angle));
        float delta_pitch_raw = Tools::NormalizeDeltaDeg(static_cast<float>(offset_pitch_angle + laser_pitch_angle));
        if (std::abs(delta_yaw_raw) > Tools::Params().minimum_angle_deg ||
          ...
          const float cmd_delta_yaw =
              std::clamp(delta_yaw_raw, -Tools::Params().max_send_delta_deg, Tools::Params().max_send_delta_deg);
          const float cmd_delta_pitch =
              std::clamp(delta_pitch_raw, -Tools::Params().pitch_abs_limit, Tools::Params().pitch_abs_limit);
          ...
          StorePendingSend(AimbotSendCommand{send_abs_pitch, send_abs_yaw, cmd_delta_pitch, cmd_delta_yaw,
                                             pitch_velocity, yaw_velocity, 0x01, frame_ts, command_enqueue_time});
    ...
    const bool do_display =
        enable_display &&
        (ui_frame_counter % static_cast<std::uint64_t>(std::max(1, Tools::Params().display_every_n_frames)) == 0);
    const bool do_gui_poll =
        enable_display &&
        (ui_frame_counter % static_cast<std::uint64_t>(std::max(1, Tools::Params().gui_poll_every_n_frames)) == 0);
    ++ui_frame_counter;

    if (do_display) {
      frame = inflight_frame.clone();
      ImageRecognize::ImageShow::ShowLockProgress(frame, g_aerial_robot_stage.load(std::memory_order_acquire),
    ...
    if (do_gui_poll) {
      ...
      if (!should_exit && current_exposure_mode != previous_exposure_mode) {
        if (current_exposure_mode == CameraTask::ExposureHotkeyController::ExposureMode::Stage3) {
    ...
    if (enable_latency_profile &&
        latency_window.frames >= static_cast<std::uint64_t>(Tools::Params().latency_print_interval_frames)) {
  ...
  if (enable_latency_profile) {
    {
      std::lock_guard<std::mutex> lk(g_send_latency_mutex);
      if (g_send_latency_window.frames > 0) PrintLatencyStats(g_send_latency_window, "窗口尾-串口链路");
  ...
  while (g_running) {
    try {
      ...
      if (read_ok) {
        ...
        g_imu_buffer.Add(ts, angles, std::chrono::milliseconds(Tools::Params().imu_buffer_max_age_ms));
      } else {
        std::this_thread::sleep_for(std::chrono::milliseconds(Tools::Params().imu_read_fail_sleep_ms));
      }
    } catch (const std::exception &e) {
...
void IMUSendThread(serial::Serial &port, Tools::ScanController &scan_controller, bool enable_send_log) {
  ...
  const auto scan_origin_hold_duration = std::chrono::milliseconds(std::max(0, Tools::Params().scan_origin_hold_ms));
  ...
  while (g_running) {
    ...
    if (scan_mode) {
      ...
      if (!g_imu_buffer.GetLatest(&latest_imu)) {
        WaitForScanStateChangeFor(std::chrono::milliseconds(Tools::Params().imu_send_idle_sleep_ms));
        continue;
      }
      ...
      Tools::ScanCommand scan_command{};
      {
        ...
        if (scan_waiting_at_origin) {
          scan_command = scan_controller.BuildOriginCommand(latest_imu.yaw, latest_imu.pitch);
        } else {
      ...
      try {
        ...
        SendAimbotCommand(port, AimbotSendCommand{scan_command.absolute_pitch_deg, scan_command.absolute_yaw_deg,
                                                  scan_command.offset_pitch_deg, scan_command.offset_yaw_deg,
                                                  scan_command.pitch_velocity_deg_per_sec,
                                                  scan_command.yaw_velocity_deg_per_sec, scan_command.aimbot_state,
                                                  scan_send_time, scan_send_time});
      } catch (const std::exception &e) {
    ...
    if (TakePendingSend(&command)) {
      if (enable_send_log) {
        std::cout << std::fixed << "发送偏角：offset_yaw=" << command.offset_yaw << "°，offset_pitch="
...
void AimbotTargetReceiveThread() {
  ...
  NetworkTask::RunAimbotTargetReceiver(g_aimbot_target, []() { return g_running.load(std::memory_order_acquire); });
}
