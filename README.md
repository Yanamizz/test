...
3. TCP 接收两类命令帧：
   `0x91 + 1Byte + 2Byte`：第 1 个 payload byte 的低 `4 bit` 写入 `game_progress`，后 `2Byte` 按大端写入 `stage_remain_time`
   `0x92 + 1Byte`：payload byte 的低 `1 bit` 表示“敌方无人机是否被反制”
4. 每次 `0x92` 的该位从 `0 -> 1` 上升沿时，阶段推进一次
5. `0x92` 的 `1` 保持不动时不会重复推进；必须先回到 `0` 再次升到 `1`
