...
- 当前主流程已新增关键 seam：`TargetTrackPipeline`、`StageRuntimeProfile`、`StagePredictorController`、`AimbotCommandArbiter`、`OverlayFrameRenderer`、`TcpStageSignalReceiver`/`TcpStageSignalClientReceiver`；扫描发送线程已新增 `ScanSendController`。后续优先沿这些 seam 深化，避免把状态重新摊回 `src/ImagePredict.cc`。
