...
### 2.6 TCP 已改为 client-only 且支持自动重连

当前实现：

- 本端不再监听 `8001/8002/8003`
- 只主动连接配置里的对端服务端
- 支持连接超时、断开后重试、空闲超时关闭
...
### 3.2 `0x020B` 是否稳定送到雷达仍要实机确认

当前 `0x0305` 的己方坐标仍主要来自：

- `serial_referee.data().ground_robot_position`

如果正式链路中雷达并不能稳定收到 `0x020B`，那：
...
- 原始日志与运行指标也足够支撑复盘
- 真正剩下的高价值问题，主要是 `loss_rate` 解释、`0x020B` 来源确认，以及更细的业务值域校验
