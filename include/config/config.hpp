...
/// 串口主协议输入模式。
constexpr auto kSerialRefereeInputMode = RefereeInputSourceMode::kFile;
...
constexpr bool kExternalTcpServerEnabled = false;
...
/// 敌方一级密钥输入模式，仅覆盖 `8002`。
constexpr auto kEnemyLevel1KeyInputMode = RefereeInputSourceMode::kFile;
...
/// 敌方二级密钥输入模式，仅覆盖 `8003`。
constexpr auto kEnemyLevel2KeyInputMode = RefereeInputSourceMode::kFile;
