...
public:
  ...
  ScanCommand BuildCommand(float imu_yaw_deg, float imu_pitch_deg) {
    ...
    const float pitch_mid_deg = 0.5f * (min_pitch_deg + max_pitch_deg);
    ...
    const float pitch_amplitude_deg =
        0.5f * pitch_span_deg * pitch_amplitude_ratio;
    ...
    const float pitch_phase_sign = yaw_direction_sign_ >= 0.0f ? 1.0f : -1.0f;
    const float scan_yaw_deg = current_yaw_deg_;
    const float scan_pitch_deg = pitch_mid_deg + pitch_phase_sign *
                                                     pitch_amplitude_deg *
    ...
    const float scan_pitch_velocity_deg_per_sec =
        ...
        ((2.0f * kPiF_) / effective_lambda_deg) * scan_yaw_velocity_deg_per_sec;
...
inline ScanController::Config::Config()
    : min_pitch_deg(70.9f),         // pitch 扫描范围下限（度）
      ...
      pitch_wavelength_percent(
          100.0f), // lambda 百分比，100% 表示单次去程恰好一周期
