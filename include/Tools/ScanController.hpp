...
public:
  ...
  ScanCommand BuildCommand(float imu_yaw_deg, float imu_pitch_deg) {
    ...
    if (config_.axis == ScanAxis::Yaw) {
      ...
      const float dt_sec = std::max(
          0.0f, std::chrono::duration_cast<std::chrono::duration<float>>(
      ...
      const float center_deg = 0.5f * (scan_min_deg + scan_max_deg);
      const float amplitude_deg = 0.25f * std::abs(scan_max_deg - scan_min_deg);
      const float min_yaw_deg =
          std::min(config_.min_yaw_deg, config_.max_yaw_deg);
      const float max_yaw_deg =
          std::max(config_.min_yaw_deg, config_.max_yaw_deg);
      ...
      const float normalized_progress =
          ...
          std::max(max_yaw_deg - min_yaw_deg, 1e-3f);
      const float scan_deg =
          center_deg +
          amplitude_deg * std::sin(2.0f * kPi * normalized_progress +
                                   (scan_forward_ ? 0.0f : kPi));
