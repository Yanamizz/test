...
class DistanceCalculator {
 public:
  float CalculateDistance(float edge_a, float edge_b) {
    ...
    const float raw_distance = FuseDistance(distance_by_h, distance_by_w, h_, w_);
 ...
 private:
  ...
  static float FuseDistance(float distance_by_h, float distance_by_w, float pixel_h, float pixel_w) {
    const float height_weight = pixel_h * pixel_h;
    const float width_weight = pixel_w * pixel_w * Params().width_weight_scale;
    return (distance_by_h * height_weight + distance_by_w * width_weight) / (height_weight + width_weight);
  }
  ...
  float FilterDistance(float raw_distance) {
    ...
    const float reset_threshold = filtered_distance_ * Params().filter_reset_ratio;
    if (std::abs(raw_distance - filtered_distance_) > reset_threshold) {
      filtered_distance_ = raw_distance;
      return raw_distance;
    }

    filtered_distance_ += Params().distance_filter_alpha * (raw_distance - filtered_distance_);
  ...
  static const TunableParams &Params() {
    ...
    static const TunableParams p{
        ...
        0.05f,   // target_width: 目标实际宽度（米）
        0.75f,   // width_weight_scale: 宽度框更容易受姿态影响，略降权
        0.35f,   // distance_filter_alpha: 一阶滤波系数，越大响应越快
        0.40f    // filter_reset_ratio: 距离突变超过该比例时重置滤波
    };
    return p;
  }
...
class LaserAngleCalculator {
 public:
  float CalculateLaserYawAngleByDistance(float distance) { return CalculateLaserAngles(distance, 0.0f, 0.0f).first; }

  float CalculateLaserPitchAngleByDistance(float distance) { return CalculateLaserAngles(distance, 0.0f, 0.0f).second; }

  std::pair<float, float> CalculateLaserAngles(float distance, float offset_yaw_deg, float offset_pitch_deg) {
    static_cast<void>(offset_yaw_deg);
    static_cast<void>(offset_pitch_deg);

    // 激光和相机的连线始终垂直于相机视线；激光在上方时，pitch 补偿为 atan(高度 / 距离)。
    const float current_pitch = ComputePitchCorrectionDeg(distance);
    const float reference_pitch = ComputePitchCorrectionDeg(Params().reference_distance_m);
    return {Params().fixed_offset_yaw, current_pitch - reference_pitch + Params().fixed_offset_pitch};
  }
 ...
 private:
  ...
  static float ComputePitchCorrectionDeg(float distance) {
    ...
    const float laser_pitch_rad = std::atan2(Params().laser_height_above_camera_m, safe_distance);
    return laser_pitch_rad * 180.0f / kPi;
  }

  static float SafeDistance(float distance) {
    if (distance <= 0.0f) return Params().reference_distance_m;
