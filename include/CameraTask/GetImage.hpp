...
public:
  ...
  loadRuntimeParams(const std::string &path = "camera_runtime_params.ini") {
    ...
    while (std::getline(file, line)) {
      ...
      try {
        ...
        std::cerr << "[GalaxyCamera] invalid exposure_time_us in " << path
                  << ": " << line << std::endl;
      }
    }
    return loaded;
  ...
  bool saveRuntimeParams(
    ...
    if (!file.is_open()) {
      std::cerr
          << "[GalaxyCamera] failed to open runtime params file for write: "
          << path << std::endl;
      return false;
    }
...
private:
  ...
  double normalizeExposureTime(double us) const {
    ...
    const GX_STATUS status =
        GXGetFloatValue(device_handle_, "ExposureTime", &exposure_range);
    if (status != GX_STATUS_SUCCESS) {
    ...
    if (exposure_range.bIncIsValid && exposure_range.dInc > 0.0) {
      value = exposure_range.dMin +
              std::round((value - exposure_range.dMin) / exposure_range.dInc) *
                  exposure_range.dInc;
      value = std::clamp(value, exposure_range.dMin, exposure_range.dMax);
    }
    return value;
  ...
  bool applyManualExposureTime(double us) {
    ...
    GX_STATUS status =
        GXSetFloatValue(device_handle_, "ExposureTime", normalized_us);
    if (status != GX_STATUS_SUCCESS) {
