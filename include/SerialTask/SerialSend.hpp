...
inline void SerialSend(serial::Serial &serial_port, float absolute_pitch, float absolute_yaw, float pitch_offset,
  ...
  SerialTask::SendAimbotFrame(serial_port, pitch_relative_angle, yaw_relative_angle, pitch_offset, yaw_offset,
                              pitch_velocity, yaw_velocity, AimbotState, AimbotTarget);
}

inline void SendAimbotFrame(serial::Serial &serial_port, float pitch_relative_angle, float yaw_relative_angle,
                            float pitch_offset, float yaw_offset, float pitch_velocity, float yaw_velocity,
                            uint8_t AimbotState, uint8_t AimbotTarget) {
  ...
  const size_t written = serial_port.write(reinterpret_cast<uint8_t *>(&aimbot_frame), sizeof(AimbotFrame_SCM_t));
